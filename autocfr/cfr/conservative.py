...
class ConservativeSolver:
    ...
    def calc_regret_and_ev(self, h, traveser, my_reach, opp_reach):
        ...
        for index, a in enumerate(h.legal_actions()):
            ...
            if p != 0:
                ...
                euclidean += math.pow(p, 2)

        if h.current_player() != traveser:
            v = 0
            for index, a in enumerate(h.legal_actions()):
                p = s.strategy[index].item()
                v += p * (self.calc_regret_and_ev(h.child(a), traveser, my_reach, opp_reach * p) - self.weight * euclidean)
            return v
        ...
        for index, a in enumerate(h.legal_actions()):
            p = s.strategy[index].item()
            child_v[a] = self.calc_regret_and_ev(h.child(a), traveser, my_reach * p, opp_reach)
            v += p * (child_v[a] + self.weight * euclidean)
    ...
    #             # s.strategy = v / np.sum(v)
    #         else:
    #             min_in_v = np.min(v)
    #             max_in_v = np.max(v)
    #             bias = max_in_v - min_in_v + 1
    #             v = np.where(v <= 0, v + bias, v)
    #             for i in range(s.na):
    #                 s.strategy[i] = v[i] / np.sum(v)
    #     else:
    #         assert not np.all(v <= 0)
